...
---Brackets (e.g. `()` / `{}` / `[]`)
M.highlights.punctuation_bracket = Highlight.new("@punctuation.bracket", { link = gruber_hl.fg0 })
---Special symbols (e.g. `{}` in string interpolation)
M.highlights.punctuation_special = Highlight.new("@punctuation.special", { link = gruber_hl.fg0 })
...
-- Original GruberDarker draws all type names in quartz; keep definition
-- sites consistent with `@type` instead of the yellow `Typedef` group.
M.highlights.type_definition = Highlight.new("@type.definition", { link = vim_hl.type })
...
M.highlights.field = Highlight.new("@field", { link = gruber_hl.fg1 })
---similar to `@field`
M.highlights.property = Highlight.new("@property", { link = gruber_hl.fg1 })
...
M.highlights.variable_builtin = Highlight.new("@variable.builtin", { link = gruber_hl.fg1 })
...
-- Without this, `@module` falls back to `Structure` (yellow bold), painting
-- namespaces like `std`/`ios` the same loud yellow as keywords. Original
-- GruberDarker keeps scope/type names in quartz.
M.highlights.module = Highlight.new("@module", { link = vim_hl.type })
M.highlights.namespace = Highlight.new("@namespace", { link = M.highlights.module })
---symbols or atoms
